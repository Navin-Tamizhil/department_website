...
const Header = () => {
  ...
  return (
    ...
    >
      {/* ---------- Top Bar with Logo ---------- */}
      <div className="w-full flex items-center justify-between pl-3 pr-3 sm:pl-6 sm:pr-6 py-3 sm:py-4">
          ...
          {navItems.map((item) => (
            <div key={item.path} className="relative group">
              <Link
                ...
                className={`${isActive(
                  item.path
                )} text-sm xl:text-base 2xl:text-lg py-2 px-2 flex items-center cursor-pointer whitespace-nowrap`}
      ...
      {/* ---------- Mobile Navigation ---------- */}
      <div
        className={`lg:hidden bg-indigo-800 shadow-xl transition-all duration-300 overflow-hidden ${
          ...
          {navItems.map((item) => (
            <div key={item.path}>
              <div className="flex items-center justify-between">
                <Link
                  to={item.path}
                  className={`block text-base sm:text-lg py-3 flex-1 ${isActive(
                    item.path
                  )}`}
              ...
              {item.sub && (
                <div
                  className={`ml-4 border-l-2 border-indigo-600 pl-4 space-y-1 transition-all duration-300 overflow-hidden ${
                    activeDropdown === item.name
                ...
                >
                  {item.sub.map((subItem) => (
                    <Link
                      key={subItem.path}
                      to={subItem.path}
                      className={`block text-sm sm:text-base py-2 ${isActive(
                        subItem.path
                      )} text-gray-200`}
