...
function CourseDescription() {
  ...
  // Group courses by Targeted Program and Year
  // Normalize keys to "1st Year", "2nd Year", etc. for ordering
  const yearOrder = ["1st Year", "2nd Year", "3rd Year", "4th Year"];

  // Create a dictionary with year as key and courses as array
  const groupedCourses = yearOrder.reduce((acc, year) => {
    acc[year] = [];
    return acc;
  }, {});

  // Fill groupedCourses by matching year substring ignoring case and trimming
  courseData.forEach((course) => {
    // Normalize course year field to lowercase for matching
    const targetYear = course["Targeted Program and Year"]?.toLowerCase() || "";

    // Find the year from yearOrder that matches the course's targeted year
    const matchedYear = yearOrder.find(
      (yr) => targetYear.includes(yr.toLowerCase())
    );

    if (matchedYear) {
      groupedCourses[matchedYear].push(course);
    } else {
      // If no match, put in "Others" or skip — here we skip
    }
  });
  ...
  return (
    <section className="container mx-auto px-6 py-10">
      ...
      {/* Render grouped courses year-wise */}
      {yearOrder.map((year) => (
        <div key={year} className="mb-10">
          <h3 className="mb-4 text-2xl font-semibold text-indigo-700">{year}</h3>
